...
2. **Executable package constraints.** Validate both required ingredients and forbidden states across the whole deck. Azula's Tainted Pact line conflicts with six Swamps, two Islands and two Mountains. Krenko's Dragon's Herald lacks its named target and necessary sacrifice colors. Tutor matching must use printed MV/type/zone restrictions: Dizzy Spell cannot transmute for Ophidian Eye (MV3); it can transmute for Gitaxian Probe (MV1). The earlier assessment incorrectly treated Probe as MV0; this was corrected against the frozen card record during the validation study. Kaalia needs entering-attacking versus attack-trigger distinctions. Conditional payment options need available resources before contributing to a castability estimate.
